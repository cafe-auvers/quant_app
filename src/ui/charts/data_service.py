...
class ChartDataController(WindowController):
    ...
    def load_history_for_timeframe(
        ...
        if timeframe == "5M":
            cached = window._load_cached_intraday_5m(symbol, window_days=window_days)
            if force_refresh or window._can_start_intraday_fetch(symbol, window_days):
                window.start_intraday_fetch(symbol, window_days=window_days)
            if cached is not None and not cached.empty:
                return cached
            if use_live_fallback:
                return download_price_history(
                    [symbol], period=f"{window_days}d", interval="5m", max_symbols=1
                )
            return pd.DataFrame()

        if timeframe == "1H":
            hourly_history = pd.DataFrame()
            if window.db_enabled and window.db_engine is not None:
                hourly_history = load_hourly_history_from_db(
                    symbol, window.db_engine, max_rows=1000
                )
            ...
            history = window._merge_chart_histories(
                hourly_history, cached_hourly, symbol
            )
            if force_refresh and use_live_fallback:
                latest = download_price_history(
                    [symbol], period="730d", interval="1h", max_symbols=1
                )
                history = window._merge_chart_histories(history, latest, symbol)
            if not history.empty:
                return history

            if window._can_start_intraday_fetch(symbol, window_days):
                window.start_intraday_fetch(symbol, window_days=window_days)
            if use_live_fallback:
                return download_price_history(
                    [symbol], period="730d", interval="1h", max_symbols=1
        ...
        if force_refresh and use_live_fallback:
            latest = window._fetch_latest_daily_bar_for_chart(symbol)
            if history.empty or latest.empty:
                fallback = download_price_history(
                    [symbol], period="1mo", interval="1d", max_symbols=1
                )
                history = window._merge_chart_histories(history, fallback, symbol)
            if not latest.empty:
                history = window._merge_chart_histories(history, latest, symbol)
