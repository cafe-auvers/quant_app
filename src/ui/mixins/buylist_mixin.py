...
class BuylistMixin:
    ...
    def refresh_execution_queue(
        ...
        *,
        ...
        target_items, missing_symbols = self._execution_queue_target_items(env, symbols, create_missing=create_missing)
        if not target_items:
            if show_log:
                if symbols is None:
        ...
        for watch_item in target_items:
            ...
            try:
                ...
                self._apply_execution_queue_item_to_buylist(queue_item, watch_item, env, buffer_pct)
    ...
    def _buylist_submit_selected_queue_order(self, env: str) -> None:
        ...
        if int(candidate.shares or 0) < 1 or float(candidate.entry_trigger or 0.0) <= 0:
            QMessageBox.warning(self, "Invalid order", f"{item.symbol} has invalid quantity or entry trigger.")
            return
        ...
        if self._has_duplicate_open_order(env, account_no, item.symbol, OrderSide.BUY, OrderIntent.ENTRY):
            QMessageBox.warning(self, "Duplicate order", f"An open BUY ENTRY order already exists for {item.symbol}.")
            return
