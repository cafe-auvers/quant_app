...
def market_session_date(now: Optional[dt.datetime] = None) -> dt.date:
...
def market_session_date_from_value(value: Any) -> Optional[dt.date]:
    ...
    if isinstance(value, str):
        ...
        if not text:
            return None
        try:
            return market_session_date_from_value(dt.datetime.fromisoformat(text))
        except ValueError:
            try:
