"""Fetch cash and holdings for KIS production accounts.
...
Design goals:
    - Only production credentials are supported.
    - PROD uses the KIS live host by default.
    - The production token uses an isolated cache file.
...
class KisConfig:
    ...
    def overseas_balance_tr_id(self) -> str:
        return (
            self.overseas_balance_tr_id_override
            or self.environment.overseas_balance_tr_id
        )
...
class KisAccountClient:
    ...
    def _request_new_access_token(self, cache_lock_held: bool = False) -> str:
        ...
        data = self._parse_response(
            response, endpoint=TOKEN_ENDPOINT, check_rt_cd=False
        )
        token = data.get("access_token")
        if not token:
            raise KisApiError(
                f"KIS token response did not contain access_token: {data}"
            )
    ...
    def get_overseas_balance(self) -> Dict[str, Any]:
        ...
        for h in all_holdings:
            ...
            if key not in by_key or h.get("evaluation_amount", 0) >= by_key[key].get(
                "evaluation_amount", 0
            ):
                by_key[key] = h
    ...
    def _get(
        self, endpoint: str, tr_id: str, params: Dict[str, str], tr_cont: str = ""
    ) -> Dict[str, Any]:
        data, _headers = self._get_with_headers(
            endpoint, tr_id=tr_id, params=params, tr_cont=tr_cont
        )
        return data
    ...
    def _get_with_headers(
        ...
        """
        try:
            return self._get_with_headers_inner(
                endpoint, tr_id=tr_id, params=params, tr_cont=tr_cont
            )
    ...
    @staticmethod
    def _normalize_overseas_holding(
        row: Dict[str, Any], exchange: str
        ...
        return {
            ...
            "quantity": quantity,
            "available_quantity": first_number(
                row, ["ord_psbl_qty", "ovrs_ord_psbl_qty"]
            ),
            ...
            "profit_loss_rate_pct": first_number(
                row, ["evlu_pfls_rt", "evlu_pfls_rate"]
            ),
            "raw": row,
        }
...
def load_config(
    environment: KisEnvironment, account_no_override: Optional[str] = None
) -> KisConfig:
    ...
    app_secret = required_env(
        f"{prefix}_APP_SECRET", fallback=legacy_prod.get("app_secret")
    )
    ...
    cano, account_product_code = split_account_no(
        account_no, default_product_code=default_product_code
    )
    ...
    return KisConfig(
        ...
        overseas_currency=os.getenv(f"{prefix}_OVERSEAS_CURRENCY", "USD")
        .strip()
        .upper()
        or "USD",
        overseas_balance_tr_id_override=os.getenv(f"{prefix}_OVERSEAS_BALANCE_TR_ID")
        or None,
    )
    ...
    *,
    ...
    for source, raw_value in raw_values:
        try:
            ...
            cano, product_code = split_account_no(
...
def split_account_no(
    account_no: str, default_product_code: str = "01"
) -> Tuple[str, str]:
    ...
    if len(cleaned) != 10 or not cleaned.isdigit():
        raise ValueError(
            "KIS account number must look like 12345678, 12345678-01, or 1234567801"
        )
...
def print_snapshot(snapshot: Dict[str, Any], show_raw: bool = False) -> None:
    ...
    print(
        f"TR_IDs     : domestic={tr_ids.get('domestic_balance')} overseas={tr_ids.get('overseas_balance')}"
    )
    ...
    if domestic:
        ...
        print(
            f"Stock evaluation        : {summary.get('stock_evaluation_krw', 0):,.0f} KRW"
        )
        print(
            f"Total evaluation        : {summary.get('total_evaluation_krw', 0):,.0f} KRW"
        )
        print(
            f"Evaluation P/L          : {summary.get('evaluation_profit_loss_krw', 0):,.0f} KRW"
        )
...
def fetch_account_snapshot(
    ...
    *,
    ...
    env = (
        environment
        if isinstance(environment, KisEnvironment)
        else KisEnvironment(str(environment).upper())
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch KIS production account cash and holdings."
    )
    parser.add_argument(
        "--env",
        choices=[item.value for item in KisEnvironment],
        default=KisEnvironment.PROD.value,
        help="KIS environment profile. Only PROD (live account) is supported.",
    )
    parser.add_argument(
        "--domestic", action="store_true", help="Fetch domestic stock balance."
    )
    parser.add_argument(
        "--overseas", action="store_true", help="Fetch overseas stock balance."
    )
    parser.add_argument(
        "--force-token",
        action="store_true",
    ...
    parser.add_argument(
        "--raw", action="store_true", help="Print raw KIS response JSON as well."
    )
    parser.add_argument(
        "--save-json", type=Path, help="Save normalized snapshot JSON to this path."
    )
    parser.add_argument(
        "--probe-product-codes",
...
def main() -> int:
    ...
    try:
        if args.probe_product_codes:
            ...
            results = probe_account_product_codes(
                environment, f"{config.cano}-{config.account_product_code}", candidates
            )
