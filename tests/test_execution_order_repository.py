...
from src.core.execution_order_record import BrokerIdentityStatus, ExecutionOrderRecord
...
def test_two_different_records_cannot_both_claim_the_same_exact_broker_order_id(tmp_path):
    ...
    repo.record_execution_order(
        engine,
        _record(client_order_id="CID-1", broker_order_id="B-1", broker_identity_status=BrokerIdentityStatus.EXACT),
    )
...
def test_ambiguous_identity_does_not_trigger_the_broker_identity_conflict_check(tmp_path):
    ...
    # extended across records.
    other = _record(client_order_id="CID-2", symbol="MSFT")
...
def test_updating_a_record_to_claim_an_already_exact_broker_order_id_elsewhere_is_rejected(tmp_path):
    ...
    other = repo.record_execution_order(engine, _record(client_order_id="CID-2", symbol="MSFT"))
    other.broker_order_id = "B-1"
