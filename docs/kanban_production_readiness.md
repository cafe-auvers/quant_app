...
| Requirement | Owning module | Persisted state | Broker action | Failure behavior | Recovery behavior | Tests | Activation criterion |
|---|---|---|---|---|---|---|---|
| I1. Separate read/write budgets, per-endpoint throttling, backoff on rate-limit responses, with the mutation-safety correction above. | `src/services/kis_request_scheduler.py` (new) | in-memory budget state | n/a (wraps calls) | Reads retry per policy; ambiguous mutations never auto-retry. | N/A | `test_rate_limited_read_backs_off_and_retries`, `test_ambiguous_mutation_failure_is_never_retried_by_the_scheduler` | Workstream 10 |
...
| J2. Emergency actions switch the gateway's own mandatory journal (B4a) to `LOCAL_EMERGENCY_JOURNAL` mode, gated by the bounded local-lease-allowance rule above — never a bypass of B4a or INV-3. | `src/services/emergency_journal.py` (new), `execution_command_gateway.py` | append-only local file (atomic, fsynced, checksummed) | `broker.cancel_order`/Sell All, still through the gateway | If the local journal write itself fails, or the local lease allowance has expired, the highest-severity external alert fires and no further destructive call is made. | The canonical DB reconciles emergency-journal entries once it returns, using the persisted reconciliation marker. | `test_emergency_exit_uses_the_local_journal_when_the_database_is_unavailable`, `test_emergency_action_is_refused_once_the_local_lease_allowance_expires`, `test_emergency_action_is_refused_if_a_handoff_was_already_pending_when_the_db_went_down`, `test_emergency_journal_reconciles_into_the_canonical_db_on_recovery`, `test_emergency_journal_write_failure_triggers_the_highest_severity_alert` | Workstream 11 |
...
| Requirement | Owning module | Persisted state | Broker action | Failure behavior | Recovery behavior | Tests | Activation criterion |
|---|---|---|---|---|---|---|---|
| K1. Alerts delivered through a channel external to the local machine, with delivery-attempt tracking, acknowledgement, retry, deduplication key, escalation. | `src/services/external_alerting.py` (new) | delivery attempt/ack log | n/a | Delivery failure retries per policy and escalates. | N/A | `test_alert_delivery_failure_retries_and_escalates` | Workstream 12 |
