...
| 14 | Activation gate closure | ACTIVE — offline evidence schemas, fail-closed validators, passive-entry consolidation, promotion separation, and current-master Gate-1 certification are complete. Gate 2 read-only KIS protocol qualification is the next release action; Gates 3-5 remain cumulative live qualifications. See `docs/activation_gate_specification.md`. |
