...
9. **Environment parsing is permissive and inconsistent.** Parse-invalid integers and floats silently use their code fallbacks, an unrecognized present Boolean becomes `false` rather than using its declared fallback, and non-finite float strings such as `nan`/`inf` are accepted. Most execution floats/integers also receive no range validation at import (`src/core/execution_config.py:22-51`). A dashboard cannot safely write these variables without typed validation and atomic worker reload rules.
...
11. **The local live envelope is active.** The audited environment is `CONTROLLED_LIVE`, with persisted active Trade Cards defining eligible symbols and a configured entry-notional cap. Symbols are application state and never environment values. Mode, ownership, mutation budgets, and idempotency controls remain operational safety controls, not ordinary dashboard settings.
...
| ENTRY-001 | `entry.breakout_price` | Daily breakout price | 3 | SAFE | no global default; required per symbol | USD/share | optional positive float persisted per plan/card | price | `>0 (CODE)` | `null (UNKNOWN)` | `0.01` | `null` | POS | broker tick; up to 4 dp below $1 | finite positive when armed | `max(breakout, ORL) < execution <= ORH`; immutable once published/locked | anytime while passive; NP+NO once Buy Today | watchlist, queue, ORB bridge, card | per-symbol persisted plan | watchlist/buylist/card | LIVE=SAME | invalid value prevents entry or invalidates the passive zone | `src/core/orb_entry_logic.py`; `src/core/execution_queue.py`; `src/services/trade_card_orb_bridge.py` | HIGH | expose as per-trade field (already present) |
| ENTRY-002 | `entry.orb_buffer_percent` | Legacy/planning breakout buffer | 3 | SAFE | `0.1` dashboard points = `0.001` fraction | percent/fraction | `orb_buffer_percent` points; `buffer_pct` fraction | percent | `0 inclusive (CODE)` | `100 inclusive (CODE)` | `0%` | `5% (RECOMMENDED_SAFETY_BOUND)` | CODE, UI max inferred safer | 0.01 pp, 2 dp | finite; divide by 100 exactly once | existing published plans retain saved buffer; active passive-order confirmation/zone use raw canonical breakout | anytime for future plans; NP for queued plan | Buy Board header, compatibility strategy/planning views, queue/card persistence | `data/settings.json:2` -> UI conversion | code default 0.001 in compatibility paths | active broker path does not use it as trigger | treating it as live trigger misdocuments execution | `src/ui/buyboard/board.py`; `src/strategy/orb/config.py`; `src/core/orb_entry_logic.py` | HIGH | expose as planning metadata; do not label as live order trigger |
| ENTRY-003 | `entry.orb_confirmation_invariants` | ORB confirmation rule | 3 | UNSAFE | complete current-session range; valid passive zone; fresh trade `> max(breakout, ORH)` | Boolean/comparisons | strict inequalities | read-only explanation | n/a | n/a | n/a | n/a | execution invariant | n/a | fail closed on incomplete/stale/invalid bars or quote | range closes first; trade and ask remain above execution at submit | internal; immutable per submitted generation | ORB queue/bridge/trading engine | active execution code | resolved and documented in `docs/current_order_logic.md` | LIVE=SAME | relaxing submits entries without valid confirmation | `src/core/orb_entry_logic.py`; `src/services/trade_card_orb_bridge.py`; `src/services/trading_engine.py` | HIGH | keep internal |
| ENTRY-004 | `entry.candidate_upgrade_margin` | Pre-order ORB display-selection margin | 3 | CONDITIONAL | `0.0` | score points | non-negative float via `max(0, value)` at pre-order comparison | score margin | `0 inclusive (CODE)` | `null (UNKNOWN)` | `0` | `null` | NN | 0.1, 1 dp | finite; non-negative | affects queue selection before order; a working-order upgrade separately requires a later timeframe and strictly higher 0.1-precision score | pre-market; NP | execution queue manager | persisted manager fallback | UI resets manager to 0 | LIVE replacement ignores margin and requires strict higher score | confusing it with replacement threshold misstates order behavior | `src/core/execution_queue.py`; `src/core/orb_entry_logic.py`; `src/services/trade_card_orb_bridge.py` | HIGH | expert-only after sync |
...
| LIFE-001 | `orders.entry_ttl_seconds` | Legacy entry lifetime setting | 8 | UNCLEAR | `15`, but new passive entries persist no deadline | seconds | int | hidden/legacy | `>0 (CODE parser)` | `null (UNKNOWN)` | `1` | `null` | SEC+ | 1 s | positive integer | not applied to finalized passive generations; EOD/user/replacement owns cancellation | redesign only | legacy compatibility | execution config | new submit explicitly passes `attempt_deadline_at=None` | LIVE=SAME | exposing it implies a 15-second behavior that no longer exists | `src/core/execution_config.py`; `src/services/entry_attempt_manager.py` | HIGH | keep hidden; retire after compatibility audit |
...
| DATA-001 | `market_data.fallback_quote_profile` | Fallback quote polling and maximum age | 11 | CONDITIONAL | poll every `2`; stale after `3` | seconds | `FALLBACK_POSITION_PRICE_POLL_SECONDS` and `QUOTE_STALE_AFTER_SECONDS` ints | seconds | `>0 (MATH)` | `null (UNKNOWN)` | poll `1`; stale `1` | `null` | SEC+ | 0.1 s, 1 dp | finite positive; current env parser does not enforce | stale age must exceed a realistic poll/fetch cadence and align WS age checks | MC; R | fallback quote poller and realtime market-data service | execution config fallback | no local env lines; architecture documents stale age 3 | LIVE=SAME | polling too slowly or accepting stale prices; overly tight values block a healthy fallback feed | `src/core/execution_config.py:209-212`; `src/services/realtime_market_data.py:121-130,260-266` | HIGH | expert-only |
...
| DATA-005 | `market_data.outage_timing` | Existing-position outage grace and ceiling | 11 | CONDITIONAL | high-risk grace15s; all-position max hold120s; max-hold0 disables the ceiling in tests | seconds | ints | seconds | `null (UNKNOWN; negatives accepted)` | `null (UNKNOWN)` | `null (UNKNOWN)` | current | UNKNOWN/BUSINESS_RULE | 1 s | finite integers; current env parser does not enforce non-negative values | high-tier grace should be <= a positive hard ceiling; supervised flag cannot disable an unattended positive ceiling | MC+NP; R; open positions require migration plan | trading engine | local `.env:110-111` | config/example same; tests inject 1/2/5/10/0 | LIVE=SAME | negative grace liquidates high-risk positions immediately; non-positive ceiling disables the all-position limit; excessive values leave positions unprotected | `src/core/execution_config.py:270-275`; `src/services/trading_engine.py:1959-1979`; `.env:110-111` | HIGH | add typed validation before any expert UI |
...
| BROKER-002 | `broker.live_execution_envelope` | Live execution mode and active-card authorization | 13 | UNSAFE | `CONTROLLED_LIVE`; Buy Today/Entry Pending Trade Cards | enum/persisted state | env mode plus Trade Card state | separate privileged operations surface only | n/a | n/a | n/a | n/a | live-trading safety | n/a | mode in DISABLED/CONTROLLED_LIVE/FULL_LIVE; exact PROD/account card must be active | requires verified WS, mutation budgets/spacing, one attempt, cap, lease/readiness | explicit Buy Today command, market session, R | controlled-live policy/gateway | mode in local environment; symbols in operational database and `data/trade_cards.json` recovery snapshot | code fallback DISABLED/empty active list; runbook | PROD-only | unauthorized production mutation | `src/core/execution_config.py`; `src/services/controlled_live_policy.py`; `src/services/trade_card_repository.py` | HIGH | keep privileged/internal |
...
| BROKER-006 | `broker.websocket_profile` | KIS WebSocket activation/capacity/reconnect profile | 13 | UNSAFE | local: enabled `true`, protocol verified `true`, mode `WEBSOCKET`, fallback `DISPLAY_ONLY`, total capacity `41`, deprecated per-channel capacities `0`; approval TTL82800s; auth retry3; reconnect1..30s ±0.5; ACK5s. Code/example fallback total capacity is `0` until measured | mixed | stable env profile + verified constant + hot-reloadable local symbol-key file | read-only readiness | n/a | n/a | n/a | n/a | broker protocol | n/a | fail closed; configured total capacity must be positive and <= verified limit41; manifest verification; symbol-key file must validate | live mode requires WEBSOCKET; fallback is display-only; malformed intraday key edits retain the last-known-good map | internal/runbook; R | KIS realtime service | stable profile in local `.env`; verified keys in gitignored `data/kis_ws_symbol_keys.json`; verified code limit | `.env.pc` matches the stable profile; symbol file is atomically updated and backed up; example/code total-capacity fallback0 | LIVE only | unverified feed authorizes entries, a fallback capacity0 blocks live composition, an unsafe key edit tears down protection, or excess subscriptions are rejected | `.env.example`; `src/core/execution_config.py`; `src/services/kis_ws_symbol_keys.py`; `src/services/kis_realtime_market_data.py` | HIGH | keep internal |
...
| INT-001 | `internal.runtime_coordination_cadence_profile` | Runtime and cross-device coordination cadence profile | 15 | UNSAFE | `ENGINE_HEARTBEAT_SECONDS=1`; active180s; standby300s; DB probe180s; lease20s; device heartbeat240s/max age>=300s; ownership proof30s; alert90s; operator20s; off-hours300s; state/board180s; remote fallback3600s; recon cache900s | seconds | env values with hard floors/coupling | read-only Health | n/a | n/a | n/a | n/a | execution timing/ownership/data-integrity | n/a | engine delay parser accepts negatives; worker clamps its sleep to at least1ms; retain coordination floors; max age >= device heartbeat+60 | raw engine delay also enters the queue-drain freshness budget; local pulse, DB lease, watchdog and failover are jointly designed | internal; R | Buy Board runtime worker, state sync/runtime repository/watchdog | execution config | `.env.example`; architecture documents the 1s engine delay | LIVE coordination absent from most unit mocks; tests inject heartbeat delay | negative/zero delay can spin the execution loop; excessive delay slows protection; split-brain, stale owner, unsafe failover | `src/core/execution_config.py:89-201`; `src/ui/buyboard/runtime_worker.py:263-265,931,998,2573-2576`; `src/services/state_sync.py:1075-1334` | HIGH | keep internal; add typed validation |
| INT-002 | `internal.protocol_identity` | Coordination/strategy protocol identities | 15 | UNSAFE | RU profile `operator-executor-sync-v7`; strategy instance `buyboard-orb-v1` | strings | protocol/ownership IDs | read-only | n/a | n/a | n/a | n/a | compatibility/idempotency | n/a | exact versioned values | every active peer must agree; strategy ownership persisted | deployment-only; R | coordination/ownership | code/env | docs/readiness | LIVE-specific | incompatible peers share store or ownership | `src/core/execution_config.py:89-93,261-266` | HIGH | keep internal |
...
| ENTRY-003 | `src/core/orb_entry_logic.py` | passive zone | `max(breakout, ORL) < execution <= ORH` | AUTH | executable candidate |
| ENTRY-003 | `src/services/trade_card_orb_bridge.py` | confirmation | fresh trade `> max(breakout, ORH)` after range close | AUTH | live breakout latch |
| ENTRY-003 | `src/services/trading_engine.py` | final submit gate | fresh trade and ask both `> execution` | DUP | immediate broker boundary |
...
| LIFE-001 | `src/core/execution_config.py` | compatibility fallback | 15 | LEGACY | retained setting |
| LIFE-001 | `src/services/entry_attempt_manager.py` | new passive generation | `attempt_deadline_at=None` | AUTH | no intraday TTL |
...
| DATA-001 | `src/core/execution_config.py:209-212` | quote fallback profile | poll2, stale3 | AUTH | REST/fallback quote service |
| DATA-001 | `src/services/realtime_market_data.py:121-130,260-266` | service defaults | stale age and poll interval from config | DERIVED | quote polling/readiness |
...
| DATA-005 | `src/core/execution_config.py:270-275` | fallbacks | 15,120; raw integer parsing permits negatives | AUTH | outage policy |
| DATA-005 | `src/services/trading_engine.py:1959-1979` | liquidation timing | negative grace is immediately reached; non-positive ceiling is disabled | DERIVED | heartbeat during disconnect |
...
| BROKER-006 | `.env:77-108` | local WS config | enabled/verified/WEBSOCKET; total capacity41; deprecated channel capacities0 | LOCAL | realtime composition |
| BROKER-006 | `src/core/execution_config.py:214-260` | WS config | switches/timings; total-capacity fallback0 | AUTH | realtime composition |
| BROKER-006 | `src/services/kis_realtime_market_data.py:53,564-567` | verified cap | 41 | AUTH | subscription validation |
| BROKER-006 | `.env.example:77-109` | example | disabled/REST display; total capacity0 | DOC | unmeasured/fail-closed setup |
...
| INT-001 | `src/core/execution_config.py:89` | `ENGINE_HEARTBEAT_SECONDS` | 1s | AUTH | Buy Board runtime worker |
| INT-001 | `src/ui/buyboard/runtime_worker.py:263-265,931,998,2573-2576` | engine consumer | cycle sleep and queue-drain freshness budget | DERIVED | execution loop/readiness |
...
| INT-002 | `src/core/execution_config.py:89-93` | RU profile | operator-executor-sync-v7 | AUTH | peer readiness |
...
| BROKER-006 | `.env.pc:81-112` | generated PC copy | enabled/verified/WEBSOCKET; total capacity41; deprecated channel capacities0 | DUP | same effective WS profile on PC |
...
| D21 | LIFE-001/LIFE-002 | config and attempt manager | legacy TTL setting retained; new passive orders have no deadline; retryable failures use 3s cooldown | active submit path wins |
...
| D31 | BROKER-004/BROKER-006 | local `.env` and `.env.pc` | matching mutation policy and effective WS profile | process environment |
...
| C17 | ENTRY-003 | Historical README/strategy buffer wording conflicted with the finalized passive-order path | **Resolved 2026-08-29:** active runtime uses raw breakout/ORL floor, `max(breakout, ORH)` confirmation, and exact passive execution price | `docs/current_order_logic.md` is canonical |
...
Environment parsing creates an additional **validation hazard**, not a separate default conflict. `_env_int` and `_env_float` silently return their code fallbacks only when conversion raises `ValueError`; `_env_bool` returns its fallback only when the variable is absent and maps every unrecognized present value to `false`; `_env_float` accepts non-finite `nan`/`inf` strings. None of the helpers reports parse, finiteness, or range errors (`src/core/execution_config.py:22-51`).
...
| ENTRY-001/ENTRY-003, ORB levels | `floor=max(breakout, ORL)`; `floor < execution <= ORH`; post-range trade `> max(breakout, ORH)` | `src/core/orb_entry_logic.py`; `src/services/trade_card_orb_bridge.py`; `src/services/trading_engine.py` | invalid zone/confirmation/quote fails closed |
...
| LIFE-001/LIFE-006 | New passive entries have no TTL; exit/legacy deadline cancellation and ORB replacement still require broker-confirmed terminal state | `src/services/entry_attempt_manager.py`; `src/services/trading_engine.py`; `src/services/execution_command_gateway.py` | no timeout or replace intent permits duplicate exposure |
...
| DATA-001 | fallback polling must run often enough for quote age to remain within the configured stale threshold | `src/core/execution_config.py:209-212`; `src/services/realtime_market_data.py:121-130,260-266` | an excessively slow poll cadence or overly tight stale age makes the fallback feed unavailable |
| DATA-002 | WebSocket execution readiness needs both broker and receive ages within bounds plus queue/skew checks | `src/services/kis_realtime_market_data.py:1523-1525,1660-1661` | quote is display-only/not execution-ready |
...
| INT-001..003 | Execution/coordination cadences, protocol identities, and sync key set jointly protect runtime progress, feed readiness, and prevent split brain or incompatible peers. They require deployment/version migrations. |
...
6. **Resolved 2026-08-29:** the active path confirms a fresh trade above `max(orb_high, breakout_price)` and submits a passive price satisfying `max(breakout_price, orb_low) < execution_price <= orb_high`; Buffer % is not the live broker trigger. See `docs/current_order_logic.md`.
